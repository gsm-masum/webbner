(function(){
  ...
  var tileLength = 30, border = 1, radius = 100, movies = { 'All': { 'results': [] } }, loaded = {}, css3dWidget; 
  ...
  function getListSuccess ( result ) {
    ...
    if ( result ) {
      if ( movies[ this.category ] ) {
        ...
        PANOLENS.Utils.TextureLoader.load( '/asset/image/tile_bg.jpg', setImageTexture.bind( this, progressManager.addLoaded.bind( progressManager ) ), undefined, progressManager.addLoaded.bind( progressManager ) );
  ...
  function getImagesSuccess ( result ) {
    ...
    var scope = this, imageArray = [], theta, vector = new THREE.Vector3();
  ...
  function generateClassifiedWall ( categories ) {
    ...
    var tiles = new THREE.Object3D(), shadeHeight = 5, staggering = 50;
    ...
    for ( var i = 0; i < categories.length; i++ ) {
      ...
      tile.addEventListener( 'hoverenter', function( event ){
        ...
        var scope = this, offset = this.material.map.offset;
  ...
  function getSpiralMovieWall () {
    ...
    var wall, amount = 36, elevation = 1.7, scale = 1.5, addingCount = 0, tile, tiles, title, theta, vector, animation, lastChain, duration, easing, initialOpacity;
      ...
      animation.onStart(function () {
        tiles.map(function(tile){
          ...
          .to({
            ...
            z: tile.spiralPosition.z}, duration*2)
